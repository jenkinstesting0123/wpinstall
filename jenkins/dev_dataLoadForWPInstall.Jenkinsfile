            ...
            parallel {
                stage("Dump_full_usbus"){
                    steps {
                      script {
                            try {
                                sh ' python3 wpinstall-sync.py --user_name ${DEV_USER_NAME} -p ${DEV_PASSWORD} -h ${DEV_HOST_NAME} -d ${DEV_DATABASE_NAME} -t ${DEV_TABLE_NAME_BUS} -f ${DEV_FILE_NAME_BUS} --getdata'
